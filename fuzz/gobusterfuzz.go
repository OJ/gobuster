...
func NewGobusterFuzz(globalopts *lib.Options, opts *OptionsFuzz) (*GobusterFuzz, error) {
	if globalopts == nil {
...
func (d *GobusterFuzz) ProcessWord(ctx context.Context, word string, progress *lib.Progress) error {
	...
	for i := 1; i <= tries; i++ {
		var err error
		statusCode, size, _, _, err = d.http.Request(ctx, url, lib.RequestOptions{})
		if err != nil {
