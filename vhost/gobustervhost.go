...
func NewGobusterVhost(globalopts *lib.Options, opts *OptionsVhost) (*GobusterVhost, error) {
	if globalopts == nil {
...
func (v *GobusterVhost) PreRun(ctx context.Context) error {
	...
	_, _, _, tmp, err := v.http.Request(ctx, v.options.URL, lib.RequestOptions{ReturnBody: true})
	if err != nil {
	...
	_, _, _, tmp, err = v.http.Request(ctx, v.options.URL, lib.RequestOptions{Host: subdomain, ReturnBody: true})
	if err != nil {
		return fmt.Errorf("unable to connect to %s: %w", v.options.URL, err)
	}
	v.baseline2 = tmp
...
func (v *GobusterVhost) ProcessWord(ctx context.Context, word string, progress *lib.Progress) error {
	...
	for i := 1; i <= tries; i++ {
		var err error
		statusCode, size, header, body, err = v.http.Request(ctx, v.options.URL, lib.RequestOptions{Host: subdomain, ReturnBody: true})
		if err != nil {
